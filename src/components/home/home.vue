    ...
    <el-container>
      <el-aside width="200px" class="aside">
        <el-menu :unique-opened="true" :router="true"> 
          ...
          <el-submenu :index="''+item1.order" v-for="(item1,index) in navList" :key='index'> <!-- index唯一标志 值不能一样 否则点击后都能展开 -->
            <template slot="title">
              <i class="el-icon-location"></i>
              <span>{{item1.authName}}</span>
            </template>
            <!-- path是路由跳转的地址 是从后台调取的 然后再更改路由组件的地址 -->
            <el-menu-item :index="item2.path" v-for="(item2,index) in item1.children" :key='index'> <!-- route ， Vue Router路径对象 -->
